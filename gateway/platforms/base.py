...
def is_no_reply_sentinel(response) -> bool:
    """Return True only for the agent's exact intentional-silence sentinel."""
        ...
        if is_network:
            ...
            for attempt in range(1, max_retries + 1):
                ...
                if self._delivery_attempted_unverified(result):
                    ...
                    # losing exact completion proof. Another retry or fallback
                    # could duplicate the accepted prefix.
                    return result
                error_str = result.error or ""
            ...
            if response:
                ...
                for _tts_index, _tts_path in enumerate(_tts_paths):
                    try:
                        ...
                        _tts_caption_delivered = bool(
                            ...
                            or (telegram_tts_caption and _tts_ok)
                        )
                ...
                if text_content and not _tts_caption_delivered:
                    ...
                    if not is_ephemeral_response and not str(
                        ...
                        try:
                            ...
                            if await asyncio.to_thread(ledger_enabled):
                                ...
                                _obligation_token = await asyncio.to_thread(
                                    record_obligation,
                                ...
                                if _obligation_token is not None:
                                    _delivery_claimed = await asyncio.to_thread(
                                        mark_attempting,
                                        _obligation_id,
                                        _obligation_token,
                                    )
                    ...
                    if (
                        ...
                        try:
                            from gateway.delivery_ledger import (
                                is_runtime_retryable_error,
                                mark_delivered,
                                mark_failed,
                            )

                            _proof = extract_provider_delivery_proof(
                                result, delivery_adapter
                            )
                            if (
                                ...
                                await asyncio.to_thread(
                                    ...
                                    _obligation_id,
                                    _obligation_token,
                                    _delivery_error or "provider delivery proof missing",
                                    retry_after=getattr(result, "retry_after", None),
                                    # A retryable outcome is ambiguous (the
                                    # provider may have accepted it) unless the
                                    # adapter names an allowlisted transient
                                    # send-path rejection, which by contract
