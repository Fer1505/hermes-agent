...
- a live gateway may reclaim only its own allowlisted transient failures after
  a platform adapter reconnects (runtime sweep), as a fresh claim generation
- rows persist their transport owner (adapter_profile) so multiplexed
  gateways never spend a disconnected bot's retry budget
"""
...
def _fail_at_runtime(oid="ob-1", error="send_path_degraded", **kw):
    """Record, cross the send boundary, and fail definitively (fork API)."""
    token = _record(oid, **kw)
    assert dl.mark_attempting(oid, token)
    assert dl.mark_failed(oid, token, error)
    return token
...
class TestSchemaMigration:
    def test_adds_adapter_profile_to_existing_ledger(self):
        ...
        assert {"claim_generation", "claim_token", "lease_due_at", "next_attempt_at"} <= columns

    def test_record_persists_transport_owner(self):
        _record(adapter_profile="reviewer")
        _record(oid="ob-2", session_key="agent:main:slack:channel:C2", chat_id="C2")
        with dl._connect() as conn:
            rows = dict(
                conn.execute(
        ...
        assert rows == {"ob-1": "reviewer", "ob-2": "default"}


class TestRuntimeFailedSweep:
    """A live gateway may reclaim only its own transient reconnect failures.

    The claim is a fresh generation exactly like a startup claim: the row goes
    back to ``pending`` with a new claim token, ``attempts`` is spent, and the
    caller crosses the send boundary with ``mark_attempting``.
    """

    def test_claims_current_process_send_path_degraded_row(self):
        _fail_at_runtime(platform="telegram")

        claimed = dl.sweep_failed_for_runtime("telegram")

        assert len(claimed) == 1
        assert claimed[0]["needs_marker"] is True
        assert claimed[0]["marker"] == dl.RECONNECTED_MARKER
        assert claimed[0]["runtime_recovery"] is True
        assert claimed[0]["attempts"] == 1
        assert _row("ob-1")["state"] == "pending"
        assert dl.mark_attempting("ob-1", claimed[0]["claim_token"]) is True
        assert _row("ob-1")["state"] == "attempting"

    def test_stale_token_cannot_use_runtime_claim(self):
        stale = _fail_at_runtime(platform="telegram")
        claimed = dl.sweep_failed_for_runtime("telegram")
        assert claimed[0]["claim_token"] != stale
        assert dl.mark_attempting("ob-1", stale) is False

    def test_permanent_failure_is_not_claimed(self):
        _fail_at_runtime(platform="telegram", error="Forbidden: bot was blocked by the user")

        assert dl.sweep_failed_for_runtime("telegram") == []
        assert _row("ob-1")["state"] == "failed"
        assert _row("ob-1")["attempts"] == 0

    def test_claim_is_platform_scoped_and_not_reclaimed_while_claimed(self):
        _fail_at_runtime(platform="telegram")
        _fail_at_runtime(
            oid="ob-2",
    ...
    def test_release_returns_unsent_claim_without_spending_attempt(self):
        _fail_at_runtime(platform="telegram")
        claimed = dl.sweep_failed_for_runtime("telegram")
        assert _row("ob-1")["attempts"] == 1

        assert dl.release_runtime_claim("ob-1", "send_path_degraded") is True

        row = _row("ob-1")
        assert row["state"] == "failed"
        assert row["attempts"] == 0
        assert row["last_error"] == "send_path_degraded"
        # Eligible again on the next reconnect.
        assert len(dl.sweep_failed_for_runtime("telegram")) == 1

    def test_release_never_touches_a_row_past_the_send_boundary(self):
        _fail_at_runtime(platform="telegram")
        claimed = dl.sweep_failed_for_runtime("telegram")
        assert dl.mark_attempting("ob-1", claimed[0]["claim_token"])

        assert dl.release_runtime_claim("ob-1", "late release") is False
        assert _row("ob-1")["state"] == "attempting"

    def test_release_is_token_and_owner_scoped(self, monkeypatch):
        _fail_at_runtime(platform="telegram")
        claimed = dl.sweep_failed_for_runtime("telegram")

        assert dl.release_runtime_claim("ob-1", "x", claim_token="stale") is False
        monkeypatch.setattr(dl, "_owner_stamp", lambda: (54321, 202))
        assert dl.release_runtime_claim("ob-1", "x") is False
        assert _row("ob-1")["state"] == "pending"
...
class TestGatewayRedeliverySweep:
    ...
    async def test_runtime_failed_redelivery_clears_resume_before_send(self):
        ...
        _fail_at_runtime(platform="slack")
        adapter = self._adapter()
        runner = self._runner(adapter)
