...
class TestSendMessageTool:
    ...
    def test_ntfy_topic_target_bypasses_channel_directory(self):
        ...
        with patch("gateway.config.load_gateway_config", return_value=config), \
             ...
             patch("tools.send_message_tool._send_to_platform", new=AsyncMock(return_value={"success": True, "message_id": "topic-1"})) as send_mock, \
    ...
    def test_cron_duplicate_target_is_skipped_and_explained(self):
        ...
        with patch.dict(
            ...
            clear=False,
             ...
             patch("tools.interrupt.is_interrupted", return_value=False), \
             patch("model_tools._run_async", side_effect=_run_async_immediately), \
             patch("tools.send_message_tool._send_to_platform", new=AsyncMock(return_value={"success": True, "message_id": "topic-2"})) as send_mock, \
    ...
    def test_mirror_receives_current_session_user_id(self):
        ...
        with patch("gateway.config.load_gateway_config", return_value=config), \
             patch("tools.interrupt.is_interrupted", return_value=False), \
             patch("model_tools._run_async", side_effect=_run_async_immediately), \
             patch("tools.send_message_tool._send_to_platform", new=AsyncMock(return_value={"success": True, "message_id": "mirror-1"})), \
    ...
    def test_media_tag_outside_allowed_roots_is_not_sent(self, tmp_path, monkeypatch):
        ...
        with patch("gateway.config.load_gateway_config", return_value=config), \
             patch("tools.interrupt.is_interrupted", return_value=False), \
             patch("model_tools._run_async", side_effect=_run_async_immediately), \
             patch("tools.send_message_tool._send_to_platform", new=AsyncMock(return_value={"success": True, "message_id": "media-filter-1"})) as send_mock, \
             patch("gateway.mirror.mirror_to_session", return_value=True):
            result = json.loads(
                send_message_tool(
                    {
    ...
    def test_duplicate_successful_send_same_turn_is_skipped(self):
        config, _telegram_cfg = _make_config()
        ...
        assert second["reason"] == "duplicate_successful_send_same_turn"
    ...
    def test_provider_success_false_without_error_fails_closed(self):
        ...
        mirror_mock.assert_not_called()
    def test_stale_explicit_telegram_target_fails_before_provider_call(self, tmp_path):
        config, _telegram_cfg = _make_config()
        cache_file = tmp_path / "channel_directory.json"
        cache_file.write_text(json.dumps({
            "updated_at": "2026-01-01T00:00:00",
        ...
        assert result["delivery_status"] == "stale"
        assert "Refusing to send to stale target telegram:-100999" in result["error"]
        send_mock.assert_not_awaited()
