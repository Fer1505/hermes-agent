...
class TestMemoryProviderSystemPromptGating:
    ...
    # Fork contract (kept through the v2026.8.27 merge): when the memory
    # toolset is gated off, the provider block is still rendered but the
    # manager is told ``tool_guidance_enabled=False`` so it omits the tool
    # instructions — the #81014 guarantee (never advertise tools the surface
    # lacks) is carried by that flag rather than by dropping memory context.
    def test_gate_off_when_memory_toolset_disabled(self):
        block = "PROVIDER_BLOCK_SENTINEL"
        agent = self._agent(
        ...
        _build(build_system_prompt, _memory_manager=agent._memory_manager,
               enabled_toolsets=None, disabled_toolsets=["memory"])
        agent._memory_manager.build_system_prompt.assert_called_with(
            tool_guidance_enabled=False
        )

    def test_gate_off_when_memory_not_in_enabled_toolsets(self):
        block = "PROVIDER_BLOCK_SENTINEL"
        agent = self._agent(
