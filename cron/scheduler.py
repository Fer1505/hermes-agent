    ...
    *,
    ...
    try:
        ...
        try:
            ...
            # Per-job fail-closed policy gates the whole fallback walk — auth
            # AND transient-network alike. A job that disallows cron fallback
            # must surface the primary failure, never degrade silently.
            if _cron_auth_fallback_disabled_for_job(_cfg, job):
                logger.error(
                    "Job '%s': primary provider resolve failed (%s) and cron "
                    "fallback is disabled for this job: %s",
                    job_id,
                    "auth" if is_auth else "transient network",
                    resolve_exc,
                )
                raise RuntimeError(
...
def _run_one_job_body(
    ...
    *,
    ...
    try:
        ...
        # Output rejection runs only on results we still own — a discarded
        # stale result above is never style/degradation-checked or delivered.
