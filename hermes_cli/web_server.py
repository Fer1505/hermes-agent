...
async def delete_session_endpoint(session_id: str, profile: Optional[str] = None):
    ...
    def _delete():
        ...
        try:
            ...
            # exists is an idempotent success: DELETEs contract is "ensure it is
            # gone", and the desktop optimistically removes the row then restores it
            # on any error. Empty-session hygiene can race the sidebar snapshot.
            sid = db.resolve_session_id(session_id)
            if not sid:
    ...
    try:
        ...
        for server in servers:
            try:
                ...
                _log.warning(
                    "Profile-create: skipping MCP server %r: %s",
                    display_name,
                    exc,
                )
                continue
