...
|---------|-------|---------|
| `browser` | `browser_back`, `browser_cdp`, `browser_click`, `browser_console`, `browser_dialog`, `browser_get_images`, `browser_navigate`, `browser_press`, `browser_scroll`, `browser_snapshot`, `browser_type`, `browser_vision`, `web_search` | Core browser automation. Includes `web_search` as a fallback for quick lookups. `browser_dialog` is gated on a reachable CDP endpoint. The unrestricted raw `browser_cdp` escape hatch also requires `browser.allow_raw_cdp: true`. High-level browser tools, the supervisor, and dialog handling remain available without that raw-tool opt-in. |
