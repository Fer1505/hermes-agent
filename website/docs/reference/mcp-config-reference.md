...
mcp_servers:
  <server_name>:
    command: "/resolved/path/to/executable"  # authorized stdio servers
    args: []
    env: {}
    # _hermes_stdio_authorization is generated by Hermes. Do not hand-edit it.
...
## Stdio executable authorization

An stdio MCP entry is executable software. Hermes starts it only when its
command, exact args, provenance, and on-disk content hashes match a current
authorization. Supported authorization paths are:

- `hermes mcp install <catalog-name>` for a reviewed catalog manifest; or
- `hermes mcp add <name> --command /path/to/native-mcp --authorize-stdio --args ...`
  for an operator-reviewed direct native executable.

The `--authorize-stdio` flag deliberately refuses shells, Python/Node
interpreters, package runners such as `npx`/`uvx`, executable scripts,
loader-variable overrides, and encoded/indirect payloads. Put those servers in
the reviewed catalog instead, where the manifest and installed files are
hashed together.

Do not copy or hand-author the `_hermes_stdio_authorization` block. Operator
authorizations also require a matching owner-only receipt under the active
Hermes profile. Missing, edited, unknown, or changed entries are quarantined
before process creation and surfaced in logs and `hermes doctor`. HTTP/OAuth
MCP entries are unaffected because they do not launch local executables.
