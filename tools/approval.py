...
# active profile home path. The resolved-absolute form is folded into the
# ~/.hermes/ patterns at detection time by _normalize_command_for_detection().
...
# ~/.hermes/config.yaml carries approval policy and permanent approvals.
# Terminal writes to it must be gated just like file-tool writes.
_HERMES_CONFIG_PATH = (
    r'(?:~\/\.hermes/|'
    r'(?:\$home|\$\{home\})/\.hermes/|'
    r'(?:\$hermes_home|\$\{hermes_home\})/)'
