...
def _get_cron_auto_delivery_target():
    ...
    platform = (
        ...
        or os.getenv("HERMES_CRON_AUTO_DELIVER_PLATFORM", "")
    ).strip().lower()
    chat_id = (
        get_session_env("HERMES_CRON_AUTO_DELIVER_CHAT_ID", "")
        or os.getenv("HERMES_CRON_AUTO_DELIVER_CHAT_ID", "")
    ).strip()
    if not platform or not chat_id:
        return None
    thread_id = (
        get_session_env("HERMES_CRON_AUTO_DELIVER_THREAD_ID", "")
        or os.getenv("HERMES_CRON_AUTO_DELIVER_THREAD_ID", "")
    ).strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
