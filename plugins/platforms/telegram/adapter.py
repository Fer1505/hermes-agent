...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _load_peer_relay_config(self) -> Dict[str, Any]:
        raw = self.config.extra.get("peer_relay") if getattr(self.config, "extra", None) else None
        ...
        try:
            ...
            default_home = _Path(os.getenv("HERMES_HOME") or ".").expanduser()
        home = _Path(raw.get("home") or os.getenv("HERMES_HOME") or default_home).expanduser()
        inbox = _Path(raw.get("inbox") or (home / "telegram_peer_relay" / "inbox")).expanduser()
        ...
        if isinstance(peers_raw, list):
            for peer in peers_raw:
                ...
                peer_home = peer.get("home")
                peer_inbox = peer.get("inbox")
                if peer_inbox:
                    inbox_path = _Path(peer_inbox).expanduser()
                elif peer_home:
                    inbox_path = _Path(peer_home).expanduser() / "telegram_peer_relay" / "inbox"
                else:
                    continue
                peers.append({"profile": peer_profile, "inbox": str(inbox_path)})

        poll_interval = raw.get("poll_interval_seconds", raw.get("poll_interval", 0.4))
        try:
            poll_interval_seconds = max(0.1, float(poll_interval))
        except (TypeError, ValueError):
    ...
    def _peer_relay_group_allowed(self, chat_id: Optional[str]) -> bool:
        ...
        if not groups:
            return False
        return "*" in groups or str(chat_id or "") in groups
    ...
    def _start_peer_relay(self) -> None:
        ...
        logger.info(
            "[%s] Telegram peer relay enabled for groups=%s inbox=%s peers=%d",
            self.name,
            ",".join(sorted(self._peer_relay.get("groups") or [])),
            inbox,
            len(self._peer_relay.get("peers") or []),
        )
    ...
    async def _peer_relay_loop(self) -> None:
        while True:
            try:
                ...
                logger.warning("[%s] Telegram peer relay poll failed: %s", self.name, err, exc_info=True)
            await asyncio.sleep(float(self._peer_relay.get("poll_interval_seconds") or 0.4))
    ...
    async def _after_final_text_response_sent(
        ...
        inbound_depth = int(raw_payload.get("relay_depth") or 0) if raw_payload else 0
        max_depth = int(self._peer_relay.get("max_depth") or 4)
        if inbound_depth >= max_depth:
            logger.info(
                "[%s] Telegram peer relay depth limit reached for chat=%s depth=%s",
                self.name,
                source.chat_id,
                inbound_depth,
            )
        ...
        for peer in peers:
            ...
            if not str(inbox):
                continue
            filename = f"{time.time_ns()}-{profile or 'telegram'}-{uuid.uuid4().hex}.json"
            target = inbox / filename
            await asyncio.to_thread(atomic_json_write, target, payload, indent=2)
            logger.info(
                "[%s] Relayed Telegram group response to peer=%s chat=%s message=%s",
                self.name,
                peer.get("profile") or target.parent,
                source.chat_id,
                result.message_id,
            )
    ...
    async def _process_peer_relay_inbox_once(self) -> int:
        ...
        for path in sorted(inbox.glob("*.json")):
            claim = path.with_suffix(path.suffix + ".processing")
            try:
    ...
    async def _handle_peer_relay_payload(self, payload: Dict[str, Any]) -> None:
        ...
        origin_name = str(payload.get("origin_display_name") or origin_profile or "Telegram peer").strip()
        ...
        event = MessageEvent(
            ...
            raw_message=payload,
            message_id=str(payload["message_id"]) if payload.get("message_id") is not None else None,
            reply_to_message_id=str(payload["trigger_message_id"]) if payload.get("trigger_message_id") is not None else None,
            channel_prompt=payload.get("channel_prompt"),
            internal=True,
        )
        logger.info(
            "[%s] Processing Telegram peer relay from %s chat=%s depth=%s",
            self.name,
            origin_name,
            chat_id,
            relay_depth,
