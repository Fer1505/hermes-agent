...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def _build_memory_uri(
        ...
        *,
        ...
        """Build a viking:// memory URI under the configured optional peer namespace."""
        # Outbox-replayed writes must land on the same URI every attempt so a
        # retry after a 409 can `replace` instead of duplicating the memory.
        slug = (
    ...
    def on_memory_write(
        ...
        try:
            ...
            if outbox_event_id:
                # Durable outbox replays must observe the failure and retry.
                raise
            return
        ...
        def _write():
            try:
                uri = self._build_memory_uri(
                    ...
                    outbox_event_id,
                    client=client,
                    timeout=_RECALL_MIN_TIMEOUT_SECONDS,
                )
                payload = {
                    "uri": uri,
                    "content": content,
