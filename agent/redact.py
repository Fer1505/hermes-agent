...
def _redact_query_string(query: str) -> str:
    ...
    Handles both `&` and `;` parameter delimiters. Sensitive keys
    (case-insensitive) have values replaced with `***`. Non-sensitive keys and
    the original delimiters pass through unchanged.
...
def _redact_sensitive_query_param_fragments(text: str) -> str:
    ...
    def _sub(m: re.Match) -> str:
        prefix = m.group(1)
        key = prefix[1:-1].lower()
        if key == "password":
            since_space = text.rfind(" ", 0, m.start()) + 1
            since_newline = text.rfind("\n", 0, m.start()) + 1
            token_start = max(since_space, since_newline)
            if "://" in text[token_start:m.start()]:
                return m.group(0)
        return f"{prefix}***"

    return _SENSITIVE_QUERY_PARAM_FRAGMENT_RE.sub(_sub, text)
...
def redact_sensitive_text(
    ...
    *,
    ...
    # Discord user/role mentions (<@snowflake_id>) — contact-metadata redaction.
