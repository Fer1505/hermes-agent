...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    if agent._memory_manager:
        try:
            # Shared effective tool gate (fork): the provider block is always
            # rendered, but its tool guidance is switched off when the memory
            # toolset is gated for this session, so the prompt never advertises
            # tools the tool surface lacks (#81014). Managers that predate the
            # keyword fall back to upstream's all-or-nothing exposure check.
            try:
                from agent.memory_manager import (
                    memory_provider_tools_enabled_for_agent as _mem_gate,
                )
            except ImportError:  # pragma: no cover - upstream-shaped memory manager
                from agent.memory_manager import (
                    memory_provider_tools_exposed as _mem_gate,
                )
            _tools_enabled = bool(_mem_gate(agent))
            try:
                _ext_mem_block = agent._memory_manager.build_system_prompt(
                    tool_guidance_enabled=_tools_enabled
                )
            except TypeError:
                _ext_mem_block = (
                    agent._memory_manager.build_system_prompt() if _tools_enabled else ""
                )
            if _ext_mem_block:
