...
def _claim_empty_response_retry(
    ...
    *,
    ...
    ``context_budget`` lets the cost-aware empty_response_guard shrink the
    per-context allowance (e.g. 3 -> 1 for expensive requests) without ever
    raising it past the fixed ceiling; the whole-turn cap always applies.
    """
    context_limit = _EMPTY_RESPONSE_CONTEXT_RETRY_LIMIT
    if context_budget is not None:
        context_limit = min(context_limit, max(0, int(context_budget)))
    total = int(getattr(agent, "_empty_content_retries_total", 0) or 0)
    if not (
        truly_empty
        and (not has_structured or prefill_exhausted)
        and agent._empty_content_retries < context_limit
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                if not agent._has_content_after_think_block(final_response):
                    ...
                    if not _deterministic_empty and _claim_empty_response_retry(
                        agent,
                        truly_empty=_truly_empty,
                        has_structured=_has_structured,
                        prefill_exhausted=_prefill_exhausted,
                        context_budget=_empty_retry_budget,
                    ):
                        ...
                        logger.warning(
                            ...
                            "retry %d/%d (turn total %d/6) in %.1fs (model=%s)",
                            agent._empty_content_retries,
                            _empty_retry_budget,
