...
def restore_primary_runtime(agent) -> bool:
    ...
    try:
        ...
        # Fallback transparency (operator directive 2026-08-19, kept through the
        # v2026.8.27 merge): an announced switch to a fallback is followed by an
        # announced switch back, exactly once, and only after an actual provider
        # fallback (a restore after a silent chain reset, or a temporary
        # `/model --once` restore, stays silent). Upstream's provenance flags
        # decide "actual provider fallback"; the fork's one-shot flag is kept
        # for callers that still set it.
        announced = bool(getattr(agent, "_fallback_switch_announced", False))
        agent._fallback_switch_announced = False
        if provider_fallback_active or announced:
            try:
                agent._emit_status(
